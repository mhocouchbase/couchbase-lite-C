...
Retained<CBLScope> CBLDatabase::getScope(slice scopeName) {
    ...
    if (auto i = _scopes.find(scopeName); i != _scopes.end()) {
        if (!exist) {
            ...
            //
            // Note: In multi db instance cases, it's not nessary that
            // all collections in the _collections map of this db instances
            // are all removed, this step is required to prevent circular reference
            // (db->collection->scope->db) when the scope if detached.
